...
<div id="content" class="container-fluid">
    <div class="card">     
        <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
            <h5 class="m-0 "><a href="{{url('admin/role/list')}}">Danh sách nhóm quyền</a></h5> 
            @if (session('status'))
            ...
            @endif          
            <a href="{{url('admin/role/add')}}" class="bg-success text-light py-2 px-3 rounded" title="Thêm mới" >Thêm mới</a>
        </div>
        <div class="card-body">
                <table class="table table-striped table-checkall">
                    ...
                    <tr class="bg-white">
                        ...
                        <td>
                            ...
                            <a href="{{route('role.delete',$role->id)}}" onclick="return confirm('Bạn có chắc chắn xóa bản ghi này ?')" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>
                        </td>
                    </tr> 
                    @endforeach
                        @else
