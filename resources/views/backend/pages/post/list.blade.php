    ...
    @if (count($items) > 0)
        @foreach ($items as $val)
            ...
            <tr>
                ...
                <td style="width: 10%"><div><img src="{{asset($val->thumbnail)}}" alt=""></div></td>
                <td style="width: 35%" class="img-in-table"><div>{{$val->name}}</div></td>
                <td style="width: 20%">{{$val->catPost->name}}</td>
